module.exports = function (RED) {
    ...
    function OracleDb(n) {
        ...
        node.on("input", function (msg) {
            ...
            node.server.query(node, query, values, resultAction, resultSetLimit, msg, node.limit);
        });
        //};
        initialize(node);
    }
    ...
    function OracleServer(n) {
        ...
        /* Option to */
        node.claimPoolConnection = function () {
            node.log("Connection claim started");
            if (!node.Connection && !node.connectionInProgress) {
                node.connectionInProgress = true;
                if (node.firstConnection) {
                ...
                node.getPool(config, (err, pool)=>{
                  if(err){
                    ...
                    if (node.reconnect) {
                      node.log("Retry connection to Oracle server in " + node.reconnectTimeout + " ms");
                      node.reconnecting = setTimeout(node.claimPoolConnection, node.reconnectTimeout);
                    }
                  }else{
        ...
        node.getPool = function (config, cb){
          if(!pool){
            ...
            oracledb.createPool(config, (err, pl) => {
              if(err){return cb(`${err} config=${JSON.stringify(config)}`);}
                    ...
                    if (err) {
                        ...
                        if (errorCode === "ORA-03113" || errorCode === "ORA-03114" || errorCode === "ORA-02396" || errorCode === "ORA-01012") {
                            ...
                            if (node.reconnect) {
                                ...
                                node.reconnecting = setTimeout(node.query, node.reconnectTimeout, requestingNode, query, values, resultAction, resultSetLimit, msg, remaining);
                            	}
                            	else{
