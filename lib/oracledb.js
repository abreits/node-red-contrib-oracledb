module.exports = function (RED) {
    ...
    function initialize(node) {
        if (node.server) {
            ...
            node.error("Oracle " + node.oracleType + " error: missing Oracle server configuration",requestingNode.msg);
        }
    }
    //
    //-- Oracle DB -----------------------------------------------------------------
    ...
    function OracleDb(n) {
        ...
        try {
            ...
            node.error("Error parsing mappings: " + err.message,node.msg);
            node.mappings = [];
        }
        ...
        node.on("input", function (msg) {
            ...
            node.server.queryWithLimit(node, query, values, resultAction, resultSetLimit, node.limit);
        });
        //};
        initialize(node);
    }
    ...
    function OracleServer(n) {
        ...
        node.claimConnection = function (requestingNode,remaining) {
            ...
            if (!node.Connection && !node.connectionInProgress) {
                ...
                oracledb.getConnection({
                    ...
                    if (err) {
                        ...
                        requestingNode.error("Oracle-server error connection to " + node.connectString + ": " + err.message,node.msg);
                        if (errorCode === "ORA-01017") {
                          requestingNode.error("WRONG Credentials " + node.connectString + ": " + err.message,node.msg);
                        }
                        else{
                          ...
                          if (node.reconnect) {
                            if(remaining > 0){
                              remaining --;
                                ...
                                node.reconnecting = setTimeout(node.claimConnection, node.reconnectTimeout , requestingNode,remaining);
                            }
                            else{
                              requestingNode.error("Fails to get connection in  " + node.limit +" times",node.msg);
        ...
        node.queryWithLimit = function (requestingNode, query, values, resultAction, resultSetLimit, remaining) {
            ...
            if (node.connection) {
                        ...
                        if (errorCode === "ORA-03113" || errorCode === "ORA-03114" || errorCode === "ORA-02396" || errorCode === "ORA-01012") {
                            ...
                            if (node.reconnect) {
                            	if(remaining > 0){
                                	...
                                	node.reconnecting = setTimeout(node.queryWithLimit, node.reconnectTimeout, requestingNode, query, values, resultAction, resultSetLimit, remaining);
                            	}
                            	else{
                                requestingNode.error("Fails to get Query connection in  " + node.limit +" times",requestingNode.msg);
                                delete node.reconnecting;
                            	}
                            }
                        }
                        else{
                          requestingNode.error("Oracle query error: " + err.message,requestingNode.msg);
                        }
                    }
                    else {
                        switch (resultAction) {
                            case "single":
                                requestingNode.msg.payload = result
                                requestingNode.send(requestingNode.msg);
                ...
                node.claimConnection(requestingNode,node.limit);
            }
        }
        ...
        node.fetchRowsFromResultSet = function (requestingNode, resultSet, maxRows) {
            resultSet.getRows(maxRows, function (err, rows) {
                if (err) {
                    requestingNode.error("Oracle resultSet error: " + err.message,requestingNode.msg);
                }
                else if (rows.length === 0) {
                    resultSet.close(function () {
                        if (err) {
                            requestingNode.error("Oracle error closing resultSet: " + err.message,requestingNode.msg);
                        }
                    });
                }
                else {
                    requestingNode.msg.payload = rows
                    requestingNode.send(requestingNode.msg);
        ...
        node.queryQueued = function () {
            while (node.connection && node.queryQueue.length > 0) {
                ...
                node.queryWithLimit(e.requestingNode, e.query, e.values, e.resultAction, e.resultSetLimit, e.sendResult,node.limit);
            }
        };
    }
