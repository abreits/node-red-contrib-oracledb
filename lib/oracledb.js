module.exports = function (RED) {
    ...
    function OracleServer(n) {
        ...
        node.claimConnection = function (remaining) {
            ...
            if (!node.Connection && !node.connectionInProgress) {
                ...
                oracledb.getConnection({
                    ...
                    if (err) {
                        ...
                        node.error("Oracle-server error connection to " + node.connectString + ": " + err.message,node.msg);
                        ...
                        if (node.reconnect) {
                        	if(remaining > 0){
                            	...
                            	node.reconnecting = setTimeout(node.claimConnection, node.reconnectTimeout , remaining);
                       			...
                       			node.error("Fails to get connection in  " + node.limit +" times",node.msg);
        ...
        node.queryWithLimit = function (requestingNode, query, values, resultAction, resultSetLimit, remaining) {
            ...
            if (node.connection) {
                ...
                node.connection.execute(query, values, options, function (err, result) {
                    if (err) {
                        requestingNode.error("Oracle query error: " + err.message,node.msg);
                        ...
                        if (errorCode === "ORA-03113" || errorCode === "ORA-03114") {
                            ...
                            if (node.reconnect) {
                            	if(remaining > 0){
                       			...
                       			node.error("Fails to get Query connection in  " + node.limit +" times",node.msg);
                       			delete node.reconnecting;
                            	}
                            }
                        }
                ...
                node.claimConnection(node.limit);
            }
        }
